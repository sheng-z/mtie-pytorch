...
class Inferencer(object):
    ...
    def buildData(self, srcBatch, goldBatch):
        srcData = [self.src_dict.convertToIdx(b,
                    mtie.Constants.UNK_WORD) for b in srcBatch]
        tgtData = None
        if goldBatch:
            tgtData = [self.tgt_dict.convertToIdx(b,
                       mtie.Constants.UNK_WORD,
                       mtie.Constants.BOS_WORD,
                       mtie.Constants.EOS_WORD) for b in goldBatch]

        return mtie.Dataset(srcData, tgtData,
            self.opt.batch_size, self.opt.cuda)

    def buildTargetTokens(self, pred, src, attn):
        tokens = self.tgt_dict.convertToLabels(pred, mtie.Constants.EOS)
        tokens = tokens[:-1]  # EOS
        if self.opt.replace_unk:
            for i in range(len(tokens)):
                if tokens[i] == mtie.Constants.UNK_WORD:
    ...
    def inferenceBatch(self, batch):
        ...
        for srcBatch_t in srcBatch.chunk(srcBatch.size(1), dim=1):
            ...
            batchPadIdx = srcBatch_t.data.squeeze(1).eq(mtie.Constants.PAD).nonzero()
        ...
        padMask = srcBatch.data.eq(mtie.Constants.PAD)
        def applyContextMask(m):
            if isinstance(m, mtie.modules.GlobalAttention):
                m.applyMask(padMask)
        ...
        if tgtBatch is not None:
            ...
            for dec_t, tgt_t in zip(decOut.transpose(0, 1), tgtBatch.transpose(0, 1)[1:].data):
                ...
                scores.masked_fill_(tgt_t.eq(mtie.Constants.PAD), 0)
                goldScores += scores
        ...
        beam = [mtie.Beam(beamSize, self.opt.cuda) for k in range(batchSize)]

        decOut = self.model.make_init_decoder_output(context)

        padMask = srcBatch.data.eq(mtie.Constants.PAD).unsqueeze(0).repeat(beamSize, 1, 1)
        ...
        for b in range(batchSize):
            ...
            valid_attn = srcBatch.transpose(0, 1).data[:, b].ne(mtie.Constants.PAD).nonzero().squeeze(1)
    ...
    def inference(self, srcBatch, goldBatch):
        ...
        #  (2) decode
        pred, predScore, attn, goldScore = self.inferenceBatch(batch)
